...
<body class="menu">
    ...
    if (isset($_SESSION['login_id'])) { //ログインしているとき
        ...
        try {
            ...
            $dptable = "SELECT timecard.id 'id',staff.name 'name',staff.rule 'rule',timecard.start 'shukkin',rules.start,timecard.end 'taikin',rules.end,timecard.date,
                        ...
                        WHERE staff.login_id=" . $login_id . " ORDER BY timecard.date DESC LIMIT :fline,10";
    ...
    ?>
        <br>
        <div>社員一覧表<?php echo "　[ID：" . $login_id . "]"; ?></div>
        ...
        <table border="1" class="table">
            ...
            <?php foreach ($sql as $row) { ?>
                <tr>
                    ...
                    <td><button type="button" onclick="location.href='/staff/time_edit.php?id=<?php echo $row['id']; ?>'">編集</a></td>
                    <td><button type="button" onclick="location.href='/staff/time_delete.php?id=<?php echo $row['id']; ?>'">削除</a></td>
                </tr>
            <?php } ?>
            ...
            if (ceil($count / 10) > 1) {
                for ($j = 1; $j <= ceil($count / 10); $j++) {  //ページ遷移ボタンの実装
                    if ($j == $nowp) {  //表示ページのみボタンではなくページ番号出力
                        ...
                        <button type="button" onclick="location.href='/staff/mykintai_list.php?id=<?php echo $j; ?>'"><?php echo "{$j}" ?></button>
            <?php }
                }
                echo "<br>";
            } ?>
        ...
        <footer id="footer">
            <button type="button" onclick="location.href='/staff/time_insert.php'">新規追加</button>
            <button type="button" onclick="location.href='/staff/menu.php'">従業員メニュー</button>
